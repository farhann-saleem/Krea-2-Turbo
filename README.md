...
Marketing Studio — **Krea-2-Turbo text-to-image** on RunPod serverless GPU.

Ping skeleton. No weights in git. Public GitHub is fine.

**Job:** prompt → image (T2I). R2 later: `comfy/comfy-models/krea2-turbo/` (~18 GB fp8). Do not re-download from Hugging Face.
...
git init
git add .
git commit -m "ping skeleton for RunPod Krea T2I"
git branch -M main
git remote add origin git@github.com:<you>/ms-runpod-krea.git
